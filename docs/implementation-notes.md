...
- 地理数据界面、非地理可视化、资源中心和管理后台作为工作台顶栏入口呈现，并按当前路由显示选中态；`/nongeo` 复用同一工作台顶栏，仅切换主体内容区并隐藏地图、图层树、右侧属性面板和底部工具面板；资源中心 `/resources` 与后台 `/admin` 均使用后台式侧边菜单和内容区，但职责分离。
...
- `/admin/` 默认进入 Dashboard；后台 Dashboard 保留用户信息、活跃用户和服务器信息。操作日志、系统设置、认证授权根据功能权限显示。Dashboard 中的数据资源、图层、栅格和数据体量相关部分归入资源中心 `/resources/`，数据导入和存量数据管理也归入资源中心，不再作为后台管理菜单项。
...
- 地图工作台左侧保留数据面板，负责当前地图会话内的数据筛选、选择、快速加载和查询加载；顶栏“资源中心”进入 `/resources/` 面板，负责 Dashboard、存量数据管理和数据导入。
...
- 资源中心 `/resources/data/inventory` 是存量数据管理入口，使用 `/api/admin/data/resources/` 查询启用和禁用资源；常规业务目录 `/api/catalog/resources/`、搜索和资源 profile/query 仍只处理 `status=active` 的数据资源。
