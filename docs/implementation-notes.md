...
- 缩略图地图使用 Web Mercator 投影，中心与主 3D 地球当前中心同步，缩放使用主图缩放小 2 级；初始中心和缩放必须来自主图 `MapViewState`，不使用硬编码默认视角。
- 中心点使用固定像素覆盖层并始终位于 2D 缩略图中心；缩略图不承载栅格符号化，栅格渲染仍由后端瓦片/PNG 服务负责。
