...
| **类型** | **数量** | **说明** |
|----|----|----|
| 可路由页面 | 12 | 登录、地理可视化、非地理可视化、成果目录、8 个后台页面 |
...
| F05 | 成果目录 | 建议新增 `/achievements` | 可路由页面 |
...
3. /map 是主工作台，非地理可视化、成果目录、管理后台都从顶部导航进入。
...
| F05 | 成果目录 | 建议新增页面 | 成果列表、成果筛选、成果详情、关联图层或资源 | `/api/achievements/`、`/api/search/`；详情和附件预览能力待确认 | 登录后访问，需浏览数据权限 | 顶部导航独立入口，承接设计文档“成果展示”要求 | 未开始 |
...
| API-05 | 成果详情、附件预览、成果关联图层加载 | F05 | 现有 `/api/achievements/` 为列表；详情深度能力待确认 | 第一阶段可用列表；需要详情页时新增详情接口或扩展列表字段 | 待确认 |
| API-06 | 全局搜索建议和分组搜索结果高亮 | F02、F05 | `/api/search/` 可搜索资源和成果，不提供建议 | 第一阶段使用普通搜索；建议能力后续再提 | 待确认 |
...
| P1-03 | 抽象顶部全局导航 | 可能使用 `/api/search/` | 地理、非地理、成果、后台入口统一 | 未开始 |
| P1-04 | 增加成果目录前端路由 `/achievements` | 使用现有成果接口 | `/achievements` 可从顶部导航进入 | 未开始 |
...
| P7-03 | 表格、基因、成果、图片四类工作区 | API-04 | 高级分析功能按接口成熟度分阶段接入 | 未开始 |

### P8 成果目录

| **任务** | **内容** | **API 影响** | **验收标准** | **状态** |
|----|----|----|----|----|
| P8-01 | 新增成果目录页面和顶部导航入口 | 复用成果列表接口 | 能浏览成果列表并返回地图 | 未开始 |
| P8-02 | 成果筛选、搜索、列表/卡片视图 | 复用 `/api/achievements/`、`/api/search/` | 成果和数据资源搜索结果分组清晰 | 未开始 |
| P8-03 | 成果详情、附件预览、关联图层 | API-05 可选 | 若后端无详情接口，先做列表级详情抽屉 | 未开始 |
...
4. 在主界面稳定后推进 P2 登录页、P8 成果目录、P7 非地理可视化。
