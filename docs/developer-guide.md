...
初始化会自动创建 `超级管理员` 和 `游客` 两个内置用户组。`超级管理员` 默认拥有全部平台功能权限；`游客` 默认仅拥有 `core.browse_data`、`core.load_vector_layer`、`core.load_raster_layer`，用于浏览和加载数据，不具备查询、导出、维护或后台管理权限。管理员新建用户时必须指定至少一个用户组，自助注册用户默认加入 `游客` 用户组。
...
后台管理位于 `/admin/`，前端基于 Ant Design Pro 组件实现，提供用户设置、操作日志、系统设置、认证授权和数据管理能力。旧版 Django Admin 已移除，不再暴露 `/admin2/`。
...
| 认证授权 | 后台提供用户创建、用户组分配、用户组增删和功能权限配置；管理员创建用户不受自助注册开关影响 |
| 数据管理 | 后台提供数据导入，按文件选择、导入配置、数据校验、数据预览和字段元数据维护完成入库 |

### 访问方式

通过浏览器访问 `/admin/` 进入管理后台。`/admin2/` 旧版 Django Admin 入口已移除。
...
- 游客用户组由系统初始化维护，默认仅保留浏览数据和加载矢量/栅格图层权限，不能删除或重命名。
- 后台认证授权入口由 `core.manage_auth` 控制；“新建用户”还需要 `core.create_user`，创建用户组还需要 `core.manage_feature_permissions`。仅具备认证授权但不具备新建用户权限的账号不能调用 `/api/users/` 创建用户；调用 `/api/users/` 创建用户时 `groupIds` 必填且至少包含一个非超级管理员用户组。
- Dashboard 入口由 `core.access_admin` 控制；活跃用户定义为统计周期内存在成功登录日志（`module=auth`、`action=login`、`status=success`）的去重用户，周期支持日、周、月。
- 操作日志和系统设置分别由 `core.view_operation_logs`、`core.manage_system_settings` 控制。
