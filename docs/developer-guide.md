...
初始化会自动创建 `超级管理员` 和 `普通用户` 两个内置用户组。`超级管理员` 默认拥有全部平台功能权限；`普通用户` 默认拥有 `core.browse_data`、`core.query_data`、`core.upload_data`、`core.load_vector_layer`、`core.load_raster_layer`，用于浏览、查询、上传和加载数据，不具备导出、存量维护或后台管理功能权限。历史 `游客` 用户组会在初始化时改名为 `普通用户`。管理员新建用户时必须指定至少一个用户组，自助注册用户默认加入 `普通用户` 用户组。
...
- 普通用户用户组由系统初始化维护，默认授予浏览、查询、上传以及加载矢量/栅格图层权限；权限允许在后台调整，用户组不能删除或重命名。历史 `游客` 用户组会自动改名为 `普通用户`。
