...
- 系统已正常运行
- 首次迁移后系统会自动创建用户名为 `admin` 的完整功能账号和 `超级管理员` 用户组；该账号是普通用户，完整功能来自用户组权限。未设置 `HUYANG_SUPERADMIN_PASSWORD` 时，随机初始密码写入业务数据目录，服务器启动时会打印该账号用户名和初始/配置密码。
...
无需输入账号密码，可使用系统维护的专用游客账号建立会话。游客账号用户名固定为 `guest`，显示名为“游客”，不可使用密码登录；账号只归属 `游客` 用户组。游客组默认具备 `core.browse_data`、`core.query_data`、`core.load_vector_layer`、`core.load_raster_layer`，不具备上传、导出、存量维护、后台管理或自定义符号化权限。管理员可在认证授权中调整游客组权限，但游客系统账号不能被删除、停用、重置密码、改组或单独授予直授权限。
...
| `core.view_group_operation_logs` | 日志权限 | 查看已配置用户组成员的日志 |
...
| `core.view_data_overview` | 概览权限 | 查看总数据情况、数据大小和条目数；超级管理员可查看按上传用户统计 |
| `core.browse_data` | 数据权限 | 浏览数据目录和资源 |
| `core.query_data` | 数据权限 | 执行数据查询 |
| `core.upload_data` | 数据权限 | 上传导入数据，不包含存量数据启停、删除或默认可视化维护 |
...
| `catalog.maintain_dataresource` | 数据权限 | 维护存量数据资源（状态、默认可视化、访问范围、删除），并兼容允许导入 |
| `raster.manage_raster_dataset` | 数据权限 | 管理栅格数据集 |
| `core.manage_feature_permissions` | 人员权限 | 配置用户组和功能权限 |
| `core.create_user` | 人员权限 | 在后台新建用户账号 |
| `core.manage_auth` | 人员权限 | 修改认证授权 |

初始化会自动创建 `超级管理员`、`普通用户` 和 `游客` 三个内置用户组。`超级管理员` 默认拥有全部平台功能权限；`普通用户` 默认拥有 `core.browse_data`、`core.query_data`、`core.upload_data`、`core.load_vector_layer`、`core.load_raster_layer`，用于注册用户浏览、查询、上传和加载数据，不具备导出、存量维护或后台管理功能权限；`游客` 默认拥有 `core.browse_data`、`core.query_data`、`core.load_vector_layer`、`core.load_raster_layer`，用于公开浏览、查询和图层加载。管理员新建用户时必须指定至少一个用户组，自助注册用户默认加入 `普通用户` 用户组。

用户最终生效的功能权限由用户组权限和单用户直授权限合并得到，再扣除用户在个人设置中主动关闭的权限。后台用户列表返回 `groupPermissions` 表示用户组继承权限，`directPermissions` 表示单独授予该用户的功能权限，`effectivePermissions` 表示最终生效权限；具备 `core.manage_auth` 和 `core.manage_feature_permissions` 的管理员可通过 `POST /api/users/{userId}/permissions/` 写入其他用户的 `directPermissions`，该操作不会修改用户所属用户组或用户组权限配置。当前登录用户不能在认证授权页修改自己的直授权限，应在用户设置中调整主动关闭权限。
...
- 用户已登录
- 用户具备 `core.upload_data` 或 `catalog.maintain_dataresource` 权限
...
formData.append("payload", JSON.stringify({
  ...
  accessGroupIds: [3],               // 额外可见用户组；上传者本人和超级管理员始终可见
}));
...
`/nongeo` 前端工作台使用独立的非地理数据合同，不复用矢量 GeoJSON 查询响应。表格数据、基因数据等非空间资源应通过下列接口读取分析摘要和明细行。
...
const response = await fetch(
  `/api/catalog/resources/${resourceId}/nongeo-analytics/`,
  { credentials: "include" },
);
const analytics = await response.json();

console.log("记录数:", analytics.summary.rowCount);
console.log("字段画像:", analytics.fields);
console.log("分类分布:", analytics.categoricalDistributions);
console.log("相关矩阵:", analytics.correlation);
```
...
analytics = session.get(
    f"{base_url}/catalog/resources/{resource_id}/nongeo-analytics/"
).json()

print(analytics["summary"]["rowCount"])
print(analytics["categoricalDistributions"])
```

该接口需要 `core.browse_data`。后端仍需根据 `DataResource.access_groups` 过滤用户可访问资源。空间数据资源调用该接口时应返回 `400`。

#### 查询非地理表格行

```javascript
// JavaScript
const response = await fetch(`/api/catalog/resources/${resourceId}/table-query/`, {
  method: "POST",
  credentials: "include",
  headers: {
    "Content-Type": "application/json",
  ...
  body: JSON.stringify({
    attributeFilters: [
      { field: "生活型（乔、灌、草）", operator: "eq", value: "灌木" },
    ],
...
console.log("返回行:", table.rows);
```

```python
# Python
table = session.post(
    f"{base_url}/catalog/resources/{resource_id}/table-query/",
    json={
        "attributeFilters": [
            {"field": "生活型（乔、灌、草）", "operator": "eq", "value": "灌木"},
        ],
...
该接口需要 `core.query_data`，只返回结构化 `rows`，不返回 `geojson` 或几何对象。属性过滤操作符沿用矢量查询的 `AttributeFilter`，但不接收 `spatialFilter`。后端应对 TopN、唯一值、相关性和分页查询设置合理上限，避免大表分析阻塞请求。
...
A: 矢量查询接口返回 GeoJSON，仅支持空间或矢量属性查询。表格数据和基因数据请使用 `/api/catalog/resources/{id}/nongeo-analytics/` 获取分析摘要，使用 `/api/catalog/resources/{id}/table-query/` 查询明细行。
...
- `GET /api/catalog/workspaces/?kind=project|topic`：列出当前用户保存的工程或专题；不传 `kind` 返回两类快照。
- `POST /api/catalog/workspaces/`：创建工程或专题，提交 `kind`、`name`、`description` 和 `snapshot`。
- `GET /api/catalog/workspaces/{id}/`：读取当前用户拥有的单个工程或专题。
- `POST /api/catalog/workspaces/{id}/`：更新工程/专题字段；提交 `{"action":"delete"}` 时删除。

`snapshot` 是前端工作台轻量 JSON 对象。后端做所有权、JSON 对象、请求体大小和嵌入原始数据检查；出现 `geojson` 或 `FeatureCollection.features` 等原始要素集合时返回 400，超大请求体返回 413。当前批次不包含共享、发布或用户组访问控制。
...
| 日志管理 | 后台通过 `/api/admin/operation-logs/` 查询真实审计日志，支持筛选、分页和 CSV 导出；通过 `/api/admin/system-logs/` 查看业务数据根目录 `logs/` 下的后台运行日志文件尾部内容。日志入口由 `core.view_operation_logs` 控制，操作日志实际可见范围由所有用户、本人、指定用户组三类日志范围权限控制；操作日志只记录用户主动行为，目录扫描、启动扫描、后台数据发现和任务内部进度进入系统日志或任务日志 |
| 系统设置 | 新版后台只展示用户可配置的 application 设置，并将修改直接写入启动时传入的源 TOML 配置文件 |
| 认证授权 | 后台提供用户创建、启用停用、删除、重置密码、用户组分配、用户组增删和功能权限配置；管理员创建用户不受自助注册开关影响 |
| 数据管理 / 存量数据 | 后台通过 `/api/admin/data/resources/` 分页查询已登记数据资源，支持快速检索、高级筛选、启用/禁用、默认可视化方案保存、访问用户组配置、删除确认以及 CSV/Excel 清单导出；上传者可进入并修改自己上传数据的可见范围 |
| 数据管理 / 工程专题 | 后台通过 `/api/admin/workspaces/` 分页查询工程和专题，支持快速检索、工程/专题筛选、启用/禁用、基础信息修改、访问用户组配置和删除确认；维护权限用户可管理全部记录，拥有者可修改自己工程专题的可见范围 |
| 数据管理 / 成果管理 | 后台通过 `/api/admin/achievements/` 分页查询成果，支持快速检索、状态/分类/来源筛选、发布状态调整、基础信息修改、访问用户组配置和删除确认 |
| 数据管理 / 数据导入 | 后台提供数据导入，按文件选择、导入配置、数据可见范围、数据校验、数据预览和字段元数据维护完成入库；入口由 `core.upload_data` 或 `catalog.maintain_dataresource` 控制 |

### 存量数据管理

存量数据管理入口为 `/admin/data/inventory`。具备 `catalog.maintain_dataresource` 的用户可以完整维护存量数据；上传者可以进入该页并修改自己上传数据的可见范围；仅具备 `catalog.export_dataresource` 的用户可查看和导出清单。列表接口返回启用和禁用资源，并包含 `sizeBytes`、`itemCount`、结构化 `uploader` 和当前用户是否可修改可见范围的 `canManageAccess`；常规业务目录 `/api/catalog/resources/` 仍只返回启用且用户可访问的数据资源，因此禁用资源会从业务查询、加载和搜索流程中隐藏，但保留在系统中。

关键接口：

- `GET /api/admin/data/resources/`：按关键词、数据类型、状态、来源、提供单位和日期范围筛选存量数据，返回可配置的数据访问用户组、上传用户、数据大小、条目数和 `canManageAccess`。仅具备 `core.upload_data` 时只返回当前用户上传的数据资源。
- `POST /api/admin/data/resources/{id}/`：通过 `action` 执行 `setStatus`、`saveVisualization`、`updateAccess`、`update` 或 `delete`。上传者本人或具备 `catalog.maintain_dataresource` 的用户可执行 `updateAccess`；其他动作需要 `catalog.maintain_dataresource`。删除必须提交与数据名称完全一致的 `confirmationName`。
- `GET /api/admin/data/resources/export/?format=csv|xlsx`：按当前筛选条件导出存量数据清单，需要 `catalog.export_dataresource`。

数据可见范围：

- 用户导入数据时，上传者本人强制可见，超级管理员用户组强制可见。
- `accessGroupIds` 表示额外可见用户组；后端会自动补齐超级管理员用户组，前端不应允许取消。
- 选择游客用户组表示无需账号即可通过游客会话浏览和查询该数据，前端必须给出醒目提示。
- 历史无上传者且无访问用户组的系统登记资源继续视为平台公共数据。
...
- 启用/禁用、默认可视化保存、访问权限配置、删除和导出都会写入 `OperationLog`，模块为“数据管理”。
- 删除用户导入的矢量/表格资源时，接口会清理对应 GeoPackage 图层或 SQLite 表；栅格等可能复用的原始文件只删除资源登记和关联图层，避免误删共享研究数据。

### 工程专题与成果管理

工程专题和成果管理入口位于 `/resources/manage/projects`、`/resources/manage/topics` 和 `/resources/manage/achievements`。页面复用同一前端管理组件，提供列表筛选、状态控制、信息编辑、访问范围配置和删除确认。工程专题使用 `active/inactive` 控制是否进入普通工作台检索和加载入口；成果使用 `draft/published/archived` 控制发布状态。

关键接口：

- `GET /api/admin/workspaces/`：按关键词、类型、状态和分页参数查询工程专题，返回 `accessGroups`、`owner` 和 `canManageAccess`。
- `POST /api/admin/workspaces/{workspaceId}/`：通过 `action` 执行 `setStatus`、`updateAccess`、`update` 或 `delete`。拥有者本人或具备 `catalog.maintain_dataresource` 的用户可执行 `updateAccess`；其他动作需要 `catalog.maintain_dataresource`。删除必须提交与工程专题名称完全一致的 `confirmationName`。
- `GET /api/admin/achievements/`：按关键词、状态、分类、来源和分页参数查询成果，返回 `accessGroups`、`owner` 和 `canManageAccess`。
- `POST /api/admin/achievements/{achievementId}/`：通过 `action` 执行 `setStatus`、`updateAccess`、`update` 或 `delete`，需要 `catalog.maintain_dataresource`。删除必须提交与成果标题完全一致的 `confirmationName`。

工程专题和成果访问范围与存量数据一致：超级管理员用户组由后端强制可见，选择游客用户组表示无需账号即可通过游客会话访问。普通业务搜索、成果列表和工作台加载接口必须继续按对象访问范围过滤，前端菜单和按钮控制只作为可用性提示，不能替代后端权限校验。

工程、专题和成果的启停、信息修改、访问范围配置和删除都应写入 `OperationLog`，模块可使用“工程管理”“专题管理”“成果管理”或统一“数据管理”，动作和说明使用中文。
...
内置账号和内置用户组不是部署运行参数，集中维护在 `backend/apps/core/configuration/builtins.py`。如需调整 `超级管理员`、`普通用户`、`游客`、`guest`、初始管理员环境变量名或默认功能权限集合，应修改该文件并补充权限初始化测试；业务视图和前端页面不得重复硬编码这些名称。

### 权限与密码管理

- 超级管理员用户组由系统维护，默认拥有全部平台功能权限；用户组不可删除或重命名，初始化的 `admin` 用户也不能从该组移除。`GET /api/groups/` 的 `lockedPermissions` 返回该组必须保留的关键功能权限。
- 普通用户用户组由系统初始化维护，默认授予浏览、查询、上传以及加载矢量/栅格图层权限；默认权限只在首次创建时应用，后续权限允许在后台调整，用户组不能删除或重命名。
- 游客用户组由系统初始化维护，默认授予浏览、查询以及加载矢量/栅格图层权限；默认权限只在首次创建时应用，后续权限允许在后台调整，用户组不能删除或重命名。专用游客账号 `guest` 由系统维护，只能通过 `POST /api/auth/guest-login/` 建立会话，不能被删除、停用、重置密码、改组或单独授予直授权限。
- 后台认证授权入口由 `core.manage_auth` 控制；“新建用户”还需要 `core.create_user`，创建用户组和配置用户组/单用户功能权限还需要 `core.manage_feature_permissions`。仅具备认证授权但不具备新建用户权限的账号不能调用 `POST /api/users/` 创建用户；调用 `POST /api/users/` 创建用户时 `groupIds` 必填且至少包含一个非超级管理员用户组。启用/停用用户通过 `POST /api/users/{userId}/` 写入 `isActive`，删除用户通过同一接口提交 `action=delete` 完成；用户组成员通过 `POST /api/users/{userId}/groups/` 更新，但当前登录用户和超级管理员用户的用户组不能在认证授权页修改；单用户功能权限和指定日志用户组通过 `POST /api/users/{userId}/permissions/` 写入 `directPermissions`、`operationLogGroupIds`，但当前登录用户不能在认证授权页修改自己的直授权限；用户组通过 `POST /api/groups/{groupId}/` 更新或提交 `action=delete` 删除；当前登录用户和初始化管理员不能被停用或删除。重置密码通过 `POST /api/users/{userId}/password/reset/` 生成随机密码，密码规则与后台创建用户一致，`generatedPassword` 仅在响应中返回一次，当前登录用户不能重置自己的密码。
- Dashboard 对所有登录用户开放；数据资源、图层数、栅格数量、用户信息和活跃用户卡片分别由 `core.view_dashboard_*_card` 权限独立控制，服务器信息整段由 `core.view_dashboard_system_card` 控制，总数据情况由 `core.view_data_overview` 控制，接口只返回已授权内容。总数据情况包含资源总数、启用资源数、总大小、总条目数和按数据类型聚合；超级管理员额外获得按上传用户聚合的资源数、大小和条目数。用户信息卡返回系统用户总数、启用账号数、停用账号数和用户组数量。活跃用户定义为统计周期内存在成功登录日志（`module=auth`、`action=login`、`status=success`）的去重用户，周期支持日、周、月。
- 日志管理和系统设置分别由 `core.view_operation_logs`、`core.manage_system_settings` 控制。`GET /api/admin/operation-logs/` 先按日志范围权限裁剪结果：`core.view_all_operation_logs` 可查看所有用户日志，`core.view_own_operation_logs` 只能查看当前用户日志，`core.view_group_operation_logs` 可查看当前用户 `operationLogGroupIds` 配置的用户组成员日志。`userId` 和 `operator` 筛选只在已授权范围内生效，不能扩大可见范围。操作日志只记录登录退出、账号权限维护、数据/工程/专题/成果的增删查改、导入导出、渲染配置和系统配置等用户主动行为；目录扫描、启动扫描、后台数据发现和任务内部进度不写入 `OperationLog`。`GET /api/admin/system-logs/?file=application.log&lines=500` 读取业务数据根目录 `logs/` 下的后台日志文件列表和指定文件尾部内容，只允许按文件名访问 `.log` 与轮转 `.log.N` 文件，不返回服务器绝对路径。
- 当前用户可通过 `POST /api/admin/profile/password/` 修改密码，通过 `POST /api/admin/profile/permissions/` 更新主动关闭的已授予权限。修改密码接口会校验当前密码、新密码至少 6 位和确认密码，并将成功或失败写入操作日志。
...
A: 使用初始化生成的 `admin` 登录后，可在新版后台“认证授权 / 用户管理”创建账号并分配具备对应管理功能权限的用户组。`admin` 是普通用户，完整功能来自 `超级管理员` 用户组权限。
