...
前端项目使用 `openapi-fetch` 和 `frontend/src/api/schema.d.ts` 进行类型安全请求。业务代码优先使用 `frontend/src/api/client.ts` 导出的 `api` 对象；新增 API 方法时应通过 `client.GET/POST` 使用 OpenAPI 中定义的路径。
...
| `core.view_dashboard_resource_card` | Dashboard 权限 | 查看 Dashboard 数据资源卡片 |
| `core.view_dashboard_layer_card` | Dashboard 权限 | 查看 Dashboard 图层数卡片 |
| `core.view_dashboard_raster_card` | Dashboard 权限 | 查看 Dashboard 栅格数量卡片 |
| `core.view_dashboard_user_card` | Dashboard 权限 | 查看 Dashboard 用户数量卡片 |
| `core.view_dashboard_active_users_card` | Dashboard 权限 | 查看 Dashboard 活跃用户卡片 |
| `core.view_dashboard_system_card` | Dashboard 权限 | 查看 Dashboard 系统信息 |
| `core.view_data_overview` | Dashboard 权限 | 查看总数据情况、数据大小和条目数；超级管理员可查看按上传用户统计 |
...
工程和专题是当前用户私有的服务器端工作台快照，用于显式保存当前图层组、顺序、可视化方案、栅格渲染元数据和地图视图。
...
`snapshot` 是前端工作台 JSON 对象，后端只做所有权和 JSON 对象校验，不解释其中图层结构。当前批次不包含共享、发布或用户组访问控制。
...
| 数据管理 / 存量数据 | 后台通过 `/api/admin/data/resources/` 分页查询已登记数据资源，支持快速检索、高级筛选、启用/禁用、默认可视化方案保存、访问用户组配置、删除确认以及 CSV/Excel 清单导出 |
| 数据管理 / 数据导入 | 后台提供数据导入，按文件选择、导入配置、数据校验、数据预览和字段元数据维护完成入库；入口由 `core.upload_data` 或 `catalog.maintain_dataresource` 控制 |

### 存量数据管理

存量数据管理入口为 `/admin/data/inventory`，由 `catalog.maintain_dataresource` 控制访问。列表接口返回启用和禁用资源，并包含 `sizeBytes`、`itemCount` 和结构化 `uploader`；常规业务目录 `/api/catalog/resources/` 仍只返回启用且用户可访问的数据资源，因此禁用资源会从业务查询、加载和搜索流程中隐藏，但保留在系统中。

关键接口：

- `GET /api/admin/data/resources/`：按关键词、数据类型、状态、来源、提供单位和日期范围筛选存量数据，返回可配置的数据访问用户组、上传用户、数据大小和条目数。
- `POST /api/admin/data/resources/{id}/`：通过 `action` 执行 `setStatus`、`saveVisualization`、`updateAccess`、`update` 或 `delete`。删除必须提交与数据名称完全一致的 `confirmationName`。
