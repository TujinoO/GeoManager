...
无需输入账号密码，可使用系统维护的专用游客账号建立会话。游客账号用户名固定为 `guest`，显示名为“游客”，不可使用密码登录；账号只归属 `游客` 角色。游客角色默认具备 `core.browse_data`、`core.query_data`、`core.load_vector_layer`、`core.load_raster_layer`，不具备上传、导出、存量维护、后台管理或自定义符号化权限。管理员可在认证授权中调整游客角色权限，但游客系统账号不能被删除、停用、重置密码、改角色或单独授予直授权限，`游客` 内置角色也不能删除或重命名。
...
初始化会自动创建 `超级管理员`、`普通用户` 和 `游客` 三个内置角色。`超级管理员` 默认拥有全部平台功能权限；`普通用户` 默认拥有 `core.browse_data`、`core.query_data`、`catalog.add_dataresource`、工程/专题增删查改、`core.load_vector_layer`、`core.load_raster_layer`，用于注册用户浏览、查询、上传、保存个人工程专题和加载数据，不具备导出、全量存量维护或后台管理功能权限；`游客` 默认拥有 `core.browse_data`、`core.query_data`、`core.load_vector_layer`、`core.load_raster_layer`，用于公开浏览、查询和图层加载。管理员新建用户时必须指定至少一个角色，自助注册用户默认加入 `普通用户` 角色。
...
| 日志管理 | 后台通过 `/api/admin/operation-logs/` 查询真实审计日志，支持筛选、分页和 CSV 导出；通过 `/api/admin/system-logs/` 查看业务数据根目录 `logs/` 下的后台运行日志文件尾部内容。日志入口由 `core.view_operation_logs` 控制，操作日志实际可见范围由所有用户、本人、指定角色三类日志范围权限控制；操作日志只记录用户主动行为，目录扫描、启动扫描、后台数据发现和任务内部进度进入系统日志或任务日志 |
| 系统设置 | 新版后台只展示用户可配置的 application 设置，并将修改直接写入启动时传入的源 TOML 配置文件 |
| 认证授权 | 后台提供用户创建、启用停用、删除、重置密码、角色分配、角色增删和功能权限配置；管理员创建用户不受自助注册开关影响 |
| 数据管理 / 存量数据 | 后台通过 `/api/admin/data/resources/` 分页查询已登记数据资源，支持快速检索、高级筛选、启用/禁用、默认可视化方案保存、访问角色配置、删除确认以及 CSV/Excel 清单导出；上传者可进入并修改自己上传数据的可见范围 |
...
- `GET /api/admin/data/resources/`：按关键词、数据类型、状态、来源、提供单位和日期范围筛选存量数据，返回可配置的数据访问角色、上传用户、数据大小、条目数和 `canManageAccess`。仅具备 `catalog.add_dataresource` 时只返回当前用户上传的数据资源。
...
- `accessGroupIds` 表示额外可见角色；后端会自动补齐超级管理员角色，前端不展示该固定项，也不允许用户取消后台强制可见规则。
...
- 普通用户角色由系统初始化维护，默认授予浏览、查询、上传以及加载矢量/栅格图层权限；默认权限只在首次创建时应用，后续权限允许在后台调整，角色不能删除或重命名。
- 游客角色由系统初始化维护，默认授予浏览、查询以及加载矢量/栅格图层权限；默认权限只在首次创建时应用，后续权限允许在后台调整，角色不能删除或重命名。专用游客账号 `guest` 由系统维护，只能通过 `POST /api/auth/guest-login/` 建立会话，不能被删除、停用、重置密码、改角色或单独授予直授权限。
...
- Dashboard 对所有登录用户开放；数据资源、图层数、栅格数量、用户信息和活跃用户卡片分别由 `core.view_dashboard_*_card` 权限独立控制，服务器信息整段由 `core.view_dashboard_system_card` 控制，总数据情况由 `core.view_data_overview` 控制，接口只返回已授权内容。总数据情况包含兼容旧版的系统总量字段，并新增 `ownUploads` 与 `visibleResources` 两个分组：`ownUploads` 仅统计当前用户上传的数据，`visibleResources` 按数据可见范围和上传者本人规则裁剪后统计当前用户可见的数据；每个分组都包含资源总数、启用资源数、总大小、总条目数和按数据类型聚合。超级管理员额外获得按上传用户聚合的资源数、大小和条目数。用户信息卡返回系统用户总数、启用账号数、停用账号数和角色数量。活跃用户定义为统计周期内存在成功登录日志（`module=auth`、`action=login`、`status=success`）的去重用户，周期支持日、周、月。
- 日志管理和系统设置分别由 `core.view_operation_logs`、`core.manage_system_settings` 控制。`GET /api/admin/operation-logs/` 先按日志范围权限裁剪结果：`core.view_all_operation_logs` 可查看所有用户日志，`core.view_own_operation_logs` 只能查看当前用户日志，`core.view_group_operation_logs` 可查看当前用户 `operationLogGroupIds` 配置的角色成员日志。`userId` 和 `operator` 筛选只在已授权范围内生效，不能扩大可见范围。操作日志只记录登录退出、账号权限维护、数据/工程/专题的增删查改、导入导出、渲染配置和系统配置等用户主动行为；目录扫描、启动扫描、后台数据发现和任务内部进度不写入 `OperationLog`。数据资源、工程/专题相关日志必须写入结构化目标字段：数据资源使用 `targetType=data_resource` 和 `DataResource.id/code/name`，工程/专题使用 `targetType=workspace_scene` 和 `WorkspaceScene.id/kind/name`；删除后仍保留删除前的目标 ID 和名称。日志接口支持 `targetType` 与 `targetId` 精确筛选，`keyword` 也会匹配目标类型、编码和名称。`GET /api/admin/system-logs/?file=application.log&lines=500` 读取业务数据根目录 `logs/` 下的后台日志文件列表和指定文件尾部内容，只允许按文件名访问 `.log` 与轮转 `.log.N` 文件，不返回服务器绝对路径。
