...
| Dashboard | 后台通过 `/api/admin/dashboard/?period=day\|week\|month` 查询数据资源、图层、栅格、用户数量和指定周期活跃用户；通过 `/api/admin/dashboard/server/` 查询 Windows、Linux、macOS 的 CPU、内存、硬盘监控快照，前端每 5 秒刷新服务器信息 |
| 操作日志 | 后台通过 `/api/admin/operation-logs/` 查询真实审计日志，支持筛选、分页和 CSV 导出 |
...
- 后台认证授权入口由 `core.manage_auth` 控制；“新建用户”还需要 `core.create_user`，创建用户组还需要 `core.manage_feature_permissions`。仅具备认证授权但不具备新建用户权限的账号不能调用 `POST /api/users/` 创建用户；调用 `POST /api/users/` 创建用户时 `groupIds` 必填且至少包含一个非超级管理员用户组。启用/停用用户通过 `POST /api/users/{userId}/` 写入 `isActive`，删除用户通过同一接口提交 `action=delete` 完成；用户组成员通过 `POST /api/users/{userId}/groups/` 更新，用户组通过 `POST /api/groups/{groupId}/` 更新或提交 `action=delete` 删除；当前登录用户和初始化管理员不能被停用或删除。重置密码通过 `POST /api/users/{userId}/password/reset/` 生成随机密码，密码规则与后台创建用户一致，`generatedPassword` 仅在响应中返回一次，当前登录用户不能重置自己的密码。
- Dashboard 入口由 `core.access_admin` 控制；活跃用户定义为统计周期内存在成功登录日志（`module=auth`、`action=login`、`status=success`）的去重用户，周期支持日、周、月。
- 操作日志和系统设置分别由 `core.view_operation_logs`、`core.manage_system_settings` 控制。`GET /api/admin/operation-logs/` 支持通过 `userId` 精确筛选指定用户日志，也支持通过 `operator` 按用户名或显示名称模糊筛选。
