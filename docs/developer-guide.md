...
后台管理位于 `/admin/`，前端基于 Ant Design Pro 组件实现，提供用户设置、操作日志、系统设置、认证授权和数据管理能力。
...
| 操作日志 | 后台通过 `/api/admin/operation-logs/` 查询真实审计日志，支持筛选、分页和 CSV 导出；日志入口由 `core.view_operation_logs` 控制，实际可见范围由所有用户、本人、指定用户组三类日志范围权限控制 |
...
- 操作日志和系统设置分别由 `core.view_operation_logs`、`core.manage_system_settings` 控制。`GET /api/admin/operation-logs/` 先按日志范围权限裁剪结果：`core.view_all_operation_logs` 可查看所有用户日志，`core.view_own_operation_logs` 只能查看当前用户日志，`core.view_group_operation_logs` 可查看当前用户 `operationLogGroupIds` 配置的用户组成员日志。`userId` 和 `operator` 筛选只在已授权范围内生效，不能扩大可见范围。
