...
- 超级管理员用户组由系统维护，默认拥有全部平台功能权限；用户组不可删除，初始化的 `admin` 用户也不能从该组移除。
- 普通用户用户组由系统初始化维护，默认授予浏览、查询、上传以及加载矢量/栅格图层权限；权限允许在后台调整，用户组不能删除或重命名。
- 游客用户组由系统初始化维护，默认授予浏览、查询以及加载矢量/栅格图层权限；权限允许在后台调整，用户组不能删除或重命名。专用游客账号 `guest` 由系统维护，只能通过 `POST /api/auth/guest-login/` 建立会话，不能被删除、停用、重置密码、改组或单独授予直授权限。
