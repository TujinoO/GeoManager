...
| `catalog.maintain_dataresource` | 数据权限 | 数据导入 |
...
| 数据管理 | 后台提供数据导入，按文件选择、导入配置、数据校验、数据预览和字段元数据维护完成入库 |
...
- 后台认证授权入口由 `core.manage_auth` 控制；“新建用户”还需要 `core.create_user`，创建用户组还需要 `core.manage_feature_permissions`。仅具备认证授权但不具备新建用户权限的账号不能调用 `/api/users/` 创建用户；调用 `/api/users/` 创建用户时 `groupIds` 必填且至少包含一个非超级管理员用户组。启用/停用用户通过 `PATCH /api/users/{userId}/` 写入 `isActive`，删除用户通过 `DELETE /api/users/{userId}/` 完成；当前登录用户和初始化管理员不能被停用或删除。重置密码通过 `POST /api/users/{userId}/password/reset/` 生成随机密码，密码规则与后台创建用户一致，`generatedPassword` 仅在响应中返回一次，当前登录用户不能重置自己的密码。
