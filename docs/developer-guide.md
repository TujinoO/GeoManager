...
| `catalog.maintain_dataresource` | 数据权限 | 维护数据资源（导入、状态、默认可视化、访问范围、删除） |
...
初始化会自动创建 `超级管理员` 和 `游客` 两个内置用户组。`超级管理员` 默认拥有全部平台功能权限；`游客` 首次创建时默认拥有 `core.browse_data`、`core.load_vector_layer`、`core.load_raster_layer`，用于浏览和加载数据，不具备查询、导出、维护或后台管理权限。游客用户组权限允许管理员在后台调整，后续初始化不会覆盖已保存配置。管理员新建用户时必须指定至少一个用户组，自助注册用户默认加入 `游客` 用户组。

用户最终生效的功能权限由用户组权限和单用户直授权限合并得到，再扣除用户在个人设置中主动关闭的权限。后台用户列表返回 `directPermissions` 表示单独授予该用户的功能权限，`effectivePermissions` 表示最终生效权限；具备 `core.manage_auth` 和 `core.manage_feature_permissions` 的管理员可通过 `POST /api/users/{userId}/permissions/` 写入其他用户的 `directPermissions`，该操作不会修改用户所属用户组或用户组权限配置。当前登录用户不能在认证授权页修改自己的直授权限，应在用户设置中调整主动关闭权限。
...
| Dashboard | 后台通过 `/api/admin/dashboard/?period=day\|week\|month` 查询数据资源、图层、栅格、用户数量和指定周期活跃用户；通过 `/api/admin/dashboard/server/` 查询 Windows、Linux、macOS 的 CPU、内存、硬盘监控快照。Dashboard 入口由 `core.access_admin` 控制，数据统计卡片由 `core.view_dashboard_*_card` 权限独立控制，服务器信息整段由 `core.view_dashboard_system_card` 控制；未授权内容不会出现在接口响应和页面中，前端每 5 秒刷新服务器信息 |
...
| 数据管理 / 数据导入 | 后台提供数据导入，按文件选择、导入配置、数据校验、数据预览和字段元数据维护完成入库 |

### 存量数据管理

存量数据管理入口为 `/admin/data/inventory`，由 `catalog.maintain_dataresource` 控制访问。列表接口返回启用和禁用资源；常规业务目录 `/api/catalog/resources/` 仍只返回启用且用户可访问的数据资源，因此禁用资源会从业务查询、加载和搜索流程中隐藏，但保留在系统中。

关键接口：

- `GET /api/admin/data/resources/`：按关键词、数据类型、状态、来源、提供单位和日期范围筛选存量数据，返回可配置的数据访问用户组。
...
- 游客用户组由系统初始化维护，首次创建时默认授予浏览数据和加载矢量/栅格图层权限；权限允许在后台调整，用户组不能删除或重命名。
...
- Dashboard 入口由 `core.access_admin` 控制；数据资源、图层数、栅格数量、用户信息和活跃用户卡片分别由 `core.view_dashboard_*_card` 权限独立控制，服务器信息整段由 `core.view_dashboard_system_card` 控制，接口只返回已授权内容。用户信息卡返回系统用户总数、启用账号数、停用账号数和用户组数量。活跃用户定义为统计周期内存在成功登录日志（`module=auth`、`action=login`、`status=success`）的去重用户，周期支持日、周、月。
