...
初始化会自动创建 `超级管理员` 和 `游客` 两个内置用户组。`超级管理员` 默认拥有全部平台功能权限；`游客` 首次创建时默认拥有 `core.browse_data`、`core.load_vector_layer`、`core.load_raster_layer`，用于浏览和加载数据，不具备查询、导出、维护或后台管理功能权限。游客用户组权限允许管理员在后台调整，后续初始化不会覆盖已保存配置。管理员新建用户时必须指定至少一个用户组，自助注册用户默认加入 `游客` 用户组。
...
- Dashboard 对所有登录用户开放；数据资源、图层数、栅格数量、用户信息和活跃用户卡片分别由 `core.view_dashboard_*_card` 权限独立控制，服务器信息整段由 `core.view_dashboard_system_card` 控制，接口只返回已授权内容。用户信息卡返回系统用户总数、启用账号数、停用账号数和用户组数量。活跃用户定义为统计周期内存在成功登录日志（`module=auth`、`action=login`、`status=success`）的去重用户，周期支持日、周、月。
