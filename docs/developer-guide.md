...
| 数据管理 / 存量数据 | 后台通过 `/api/admin/data/resources/` 分页查询当前用户可见或本人上传的已登记数据资源，支持快速检索、高级筛选、启用/禁用、默认可视化方案保存、访问角色配置、删除确认以及 CSV/Excel 清单导出；超级管理员可查看和维护全部资源；非超级管理员主体不会收到超级管理员访问角色；上传者可进入并修改自己上传数据的可见范围 |
...
存量数据管理入口为 `/admin/data/inventory`。具备 `catalog.view_dataresource`、`catalog.change_dataresource`、`catalog.delete_dataresource` 或 `catalog.export_dataresource` 的用户可以查看当前用户可见或本人上传且符合筛选条件的存量数据；超级管理员角色可查看全部存量数据；仅具备 `catalog.add_dataresource` 的上传者可以进入该页查看自己上传的数据并修改其可见范围。列表接口返回启用和禁用资源，并包含 `sizeBytes`、`itemCount`、结构化 `uploader` 和当前用户是否可修改可见范围的 `canManageAccess`；非超级管理员主体不会收到超级管理员访问角色，但后台仍强制保留该访问范围；常规业务目录 `/api/catalog/resources/` 仍只返回启用且用户可访问的数据资源，因此禁用资源会从业务查询、加载和搜索流程中隐藏，但保留在系统中。

关键接口：

- `GET /api/admin/data/resources/`：按关键词、数据类型、状态、来源、提供单位和日期范围筛选当前用户可见或本人上传的存量数据，返回当前主体可见的数据访问角色、上传用户、数据大小、条目数和 `canManageAccess`。超级管理员可查看全部资源；仅具备 `catalog.add_dataresource` 时只返回当前用户上传的数据资源。若资源维护人是当前主体不可见的超级管理员用户，`maintainer` 返回空字符串且 `uploader` 返回 `null`。
- `POST /api/admin/data/resources/{id}/`：通过 `action` 执行 `setStatus`、`saveVisualization`、`updateAccess`、`update` 或 `delete`。目标数据必须对当前用户可见或由当前用户上传；不可见数据按不存在处理；超级管理员不受对象可见范围限制。上传者本人或具备 `catalog.change_dataresource` 的用户可执行 `updateAccess`；启停、默认可视化和普通编辑需要 `catalog.change_dataresource`；删除需要 `catalog.delete_dataresource` 并提交与数据名称完全一致的 `confirmationName`。
...
- `accessGroupIds` 表示额外可见角色；后端会自动补齐超级管理员角色。非超级管理员主体的访问角色列表不返回超级管理员角色，用户不能选择或取消后台强制可见规则。
