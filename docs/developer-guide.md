...
| `catalog.add_achievement` | 数据权限 | 新增成果 |
| `catalog.view_achievement` | 数据权限 | 查看成果 |
| `catalog.change_achievement` | 数据权限 | 编辑成果 |
| `catalog.delete_achievement` | 数据权限 | 删除成果 |
| `catalog.maintain_dataresource` | 数据权限 | 历史维护权限，仅保留为已有权限记录，不再作为数据 CRUD 接口放行条件 |
...
初始化会自动创建 `超级管理员`、`普通用户` 和 `游客` 三个内置用户组。`超级管理员` 默认拥有全部平台功能权限；`普通用户` 默认拥有 `core.browse_data`、`core.query_data`、`catalog.add_dataresource`、工程/专题增删查改、`catalog.view_achievement`、`core.load_vector_layer`、`core.load_raster_layer`，用于注册用户浏览、查询、上传、保存个人工程专题和加载数据，不具备导出、全量存量维护或后台管理功能权限；`游客` 默认拥有 `core.browse_data`、`core.query_data`、`catalog.view_achievement`、`core.load_vector_layer`、`core.load_raster_layer`，用于公开浏览、查询、成果查看和图层加载。管理员新建用户时必须指定至少一个用户组，自助注册用户默认加入 `普通用户` 用户组。
...
成果接口使用独立 CRUD 权限：查看需要 `catalog.view_achievement`，新增需要 `catalog.add_achievement`，编辑需要 `catalog.change_achievement`，删除需要 `catalog.delete_achievement`。普通查看只返回已发布成果；具备成果新增、编辑或删除权限的管理用户可通过 `status` 参数筛选草稿、已发布或已下架成果。
...
管理端可通过 `POST /api/achievements/` 新增成果，通过 `GET /api/achievements/{id}/` 查看详情，通过 `POST /api/achievements/{id}/` 更新字段；提交 `{"action":"delete"}` 删除成果。
...
存量数据管理入口为 `/admin/data/inventory`。具备 `catalog.view_dataresource`、`catalog.change_dataresource`、`catalog.delete_dataresource` 或 `catalog.export_dataresource` 的用户可以查看符合筛选条件的存量数据；仅具备 `catalog.add_dataresource` 的上传者可以进入该页查看自己上传的数据并修改其可见范围。列表接口返回启用和禁用资源，并包含 `sizeBytes`、`itemCount`、结构化 `uploader` 和当前用户是否可修改可见范围的 `canManageAccess`；常规业务目录 `/api/catalog/resources/` 仍只返回启用且用户可访问的数据资源，因此禁用资源会从业务查询、加载和搜索流程中隐藏，但保留在系统中。

关键接口：

- `GET /api/admin/data/resources/`：按关键词、数据类型、状态、来源、提供单位和日期范围筛选存量数据，返回可配置的数据访问用户组、上传用户、数据大小、条目数和 `canManageAccess`。仅具备 `catalog.add_dataresource` 时只返回当前用户上传的数据资源。
- `POST /api/admin/data/resources/{id}/`：通过 `action` 执行 `setStatus`、`saveVisualization`、`updateAccess`、`update` 或 `delete`。上传者本人或具备 `catalog.change_dataresource` 的用户可执行 `updateAccess`；启停、默认可视化和普通编辑需要 `catalog.change_dataresource`；删除需要 `catalog.delete_dataresource` 并提交与数据名称完全一致的 `confirmationName`。
...
- 日志管理和系统设置分别由 `core.view_operation_logs`、`core.manage_system_settings` 控制。`GET /api/admin/operation-logs/` 先按日志范围权限裁剪结果：`core.view_all_operation_logs` 可查看所有用户日志，`core.view_own_operation_logs` 只能查看当前用户日志，`core.view_group_operation_logs` 可查看当前用户 `operationLogGroupIds` 配置的用户组成员日志。`userId` 和 `operator` 筛选只在已授权范围内生效，不能扩大可见范围。操作日志只记录登录退出、账号权限维护、数据/工程/专题/成果的增删查改、导入导出、渲染配置和系统配置等用户主动行为；目录扫描、启动扫描、后台数据发现和任务内部进度不写入 `OperationLog`。数据资源、工程/专题和成果相关日志必须写入结构化目标字段：数据资源使用 `targetType=data_resource` 和 `DataResource.id/code/name`，工程/专题使用 `targetType=workspace_scene` 和 `WorkspaceScene.id/kind/name`，成果使用 `targetType=achievement` 和 `Achievement.id/code/title`；删除后仍保留删除前的目标 ID 和名称。日志接口支持 `targetType` 与 `targetId` 精确筛选，`keyword` 也会匹配目标类型、编码和名称。`GET /api/admin/system-logs/?file=application.log&lines=500` 读取业务数据根目录 `logs/` 下的后台日志文件列表和指定文件尾部内容，只允许按文件名访问 `.log` 与轮转 `.log.N` 文件，不返回服务器绝对路径。
