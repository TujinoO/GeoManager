...
export type ClientOptions = {
    baseUrl: `${string}://${string}` | 'http://localhost:8000' | 'https://api.huyang.example.com' | (string & {});
};
...
export type UserPermissions = {
    /**
     * 当前登录会话是否具备至少一项运行概览、日志、系统设置、备份或认证授权等运维权限。普通用户、科研用户和游客在仅有数据业务权限时为 false，前端隐藏后台入口；个人设置仍可独立访问。
     */
...
export type AdminSettingsUpdateRequest = {
    /**
     * 更新后的系统显示名称；提交已知历史平台名称时会规范化并持久化为当前确认的全球平台名称
     */
    systemName?: string;
    /**
     * 是否开放自助注册
...
/**
 * 草稿仅创建者和管理员可见，已发布成果按访问角色共享；游客角色表示公开成果，游客及其他已登录角色均可见
 */
...
export type AdminDataResourceAccessGroup = {
    ...
    name: string;
    /**
     * 是否为系统内置游客用户组；选择该组表示对象公开，既可通过游客会话访问，其他已登录角色也保留访问权
     */
    isGuest: boolean;
    /**
...
export type AdminDataResourceUpdateRequest = {
    ...
    /**
     * update 时写入的数据资源显示名称；后端去除首尾空白，不修改数据编号、物理存储标识或默认图层名称
     */
    name?: string;
    /**
    ...
    /**
     * updateAccess 或 update 时写入的额外可见用户组 ID 列表；调用方不能选择超级管理员用户组，后端会强制补齐超级管理员用户组，上传者本人不依赖用户组也始终可见。包含游客用户组时表示该数据公开，游客会话和其他已登录角色均可访问。
     */
    accessGroupIds?: Array<number>;
    /**
...
export type AdminWorkspaceSceneUpdateRequest = {
    ...
    description?: string;
    /**
     * updateAccess 或 update 时写入的额外可见用户组 ID 列表；不能选择超级管理员用户组，后端会强制保留管理员访问，所属用户本人始终可见。包含游客用户组时表示该工程公开，游客会话和其他已登录角色均可访问。
     */
    accessGroupIds?: Array<number>;
    /**
 ...
 * 字段在分析视图中的推断角色
 */
export type NonGeoFieldRole = 'identifier' | 'category' | 'measure' | 'date' | 'text' | 'coordinate' | 'unknown';
...
export type NonGeoTableField = {
     ...
     * 当前结果中最多三个非空样例值
     */
    sampleValues: Array<NonGeoScalar>;
    /**
     * 后台字段元数据说明；没有说明时为空字符串
     */
    description: string;
};
...
export type NonGeoAnalyticsSummary = {
    /**
     * 资源真实总记录数
     */
    rowCount: number;
...
export type NonGeoCategoricalDistributionItem = {
    value: NonGeoScalar;
    /**
     * 分类值计数
     */
...
export type NonGeoCategoricalDistribution = {
     ...
     * 按数量降序返回的前十二个分类值
     */
    items: Array<NonGeoCategoricalDistributionItem>;
};
...
export type NonGeoCorrelation = {
    /**
     * 参与相关性计算的前六个数值字段
     */
    fields: Array<string>;
    /**
     * 与 fields 顺序一致的相关系数矩阵
     */
...
export type NonGeoAnalyticsResponse = {
    resource: DataResource;
    summary: NonGeoAnalyticsSummary;
    /**
     * 字段画像
     */
    ...
    /**
     * 至少存在两个数值字段时返回相关系数矩阵，否则为 null
     */
    correlation: NonGeoCorrelation | null;
...
export type GetResourceNonGeoAnalysisData = {
    body?: never;
    path: {
        /**
         * 资源 ID
...
export type GetResourceNonGeoAnalysisErrors = {
    /**
     * 请求错误
     */
    400: ErrorResponse;
...
export type GetResourceNonGeoAnalysisError = GetResourceNonGeoAnalysisErrors[keyof GetResourceNonGeoAnalysisErrors];

export type GetResourceNonGeoAnalysisResponses = {
    /**
     * 分析成功
     */
...
export type GetResourceNonGeoAnalysisResponse = GetResourceNonGeoAnalysisResponses[keyof GetResourceNonGeoAnalysisResponses];

export type QueryResourceNonGeoTableData = {
...
export type QueryResourceNonGeoTableErrors = {
    /**
     * 请求错误
     */
    400: ErrorResponse;
...
export type QueryResourceNonGeoTableError = QueryResourceNonGeoTableErrors[keyof QueryResourceNonGeoTableErrors];

export type QueryResourceNonGeoTableResponses = {
    /**
     * 查询成功
     */
...
export type QueryResourceNonGeoTableResponse = QueryResourceNonGeoTableResponses[keyof QueryResourceNonGeoTableResponses];
...
export type CreateResultArtifactData = {
    body: {
        /**
         * PNG、JPG/JPEG、PDF、CSV 或 XLSX 成果文件；文件扩展名必须与后端识别的真实内容格式一致
         */
        file: Blob | File;
        /**
...
export type QueryResourceErrors = {
    ...
    413: ErrorResponse;
    /**
     * 服务当前没有可用的重型任务执行槽；可按 Retry-After 退避重试
     */
    503: ErrorResponse;
};
