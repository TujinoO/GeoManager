...
import type { LoadedLayer, LoadedVectorLayer, GeoJsonGeometry } from '../types';
import type { VectorSymbolization } from '../symbolization';
import { sourceIdFor, clamp } from '../utils/geometry';
import { getMapState } from './mapState';
import { stateColor, stateNumber, upsertLayer, removeStyleLayer, buildVectorPaintProperties } from './styleHelpers';
import { removeVectorInteraction } from './featureInteraction';

export function addLoadedStyleLayers(map: mapboxgl.Map, sourceId: string, layer: LoadedVectorLayer) {
  const style = layer.symbolization;
  const layerOpacity = clamp(style.opacity / 100, 0, 1);
  const { circleOpacity, circleStrokeOpacity, symbolIconOpacity, symbolTextOpacity, lineOpacity, fillOpacity } =
...
export function removeLayerGroup(map: mapboxgl.Map, sourceId: string, layerIds: string[]) {
  layerIds.forEach((id) => {
...
export function reorderLoadedStyleLayers(map: mapboxgl.Map, layers: LoadedLayer[]) {
