...
describe('polygonGeometry', () => {
  it('closes the ring', () => {
    const geo = polygonGeometry([[0, 0], [1, 0], [1, 1]]);
    const coords = (geo as { coordinates: number[][][] }).coordinates[0];
...
describe('sourceIdFor', () => {
  it('prefixes with loaded-', () => expect(sourceIdFor('abc')).toBe('loaded-abc'));
});
...
describe('extractCoordinates', () => {
  ...
  it('recursively extracts nested coordinates', () => {
    const points: [number, number][] = [];
    extractCoordinates([[[1, 2], [3, 4]]], points);
...
describe('combinedFeatureBounds', () => {
  ...
  it('returns bounds for features with coordinates', () => {
    const fc = {
      ...
      features: [
        { type: 'Feature' as const, properties: {}, geometry: { type: 'Point', coordinates: [10, 20] } },
        { type: 'Feature' as const, properties: {}, geometry: { type: 'Point', coordinates: [30, 40] } },
      ],
    };
...
describe('boundsFromImageCoordinates', () => {
  ...
  it('returns bounds from coordinates', () => {
    const bounds = boundsFromImageCoordinates([[10, 20], [30, 40]]);
    expect(bounds).not.toBeNull();
    expect(bounds!.getWest()).toBe(10);
    expect(bounds!.getEast()).toBe(30);
  });
